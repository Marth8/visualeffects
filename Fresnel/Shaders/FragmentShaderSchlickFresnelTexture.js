const fragmentShaderSchlickFresnelTextureString =
`
...
void main() {
    ...
    vec3 refractColor = result * vec3(texture2D(material.diffuse, vTexCoord));

    // compute reflect color
    vec3 incident = normalize(xPosition - uEyePosition);
    vec3 reflect = reflect(incident, normal);
    vec3 reflectColor = vec3(textureCube(skybox, reflect)) * result;

    // set f0 for dialetics (approximation of F0 value)
    vec3 F0 = vec3(0.04);
    F0 = mix(F0, vec3(texture2D(material.diffuse, vTexCoord)), material.metalness);
...
export default fragmentShaderSchlickFresnelTextureString